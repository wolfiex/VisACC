...
window.simulation = d3.forceSimulation()
    .force("charge", d3.forceManyBody().strength(function(d){return both.has(d.id)? -100:-300}))//- 18 -550 -32 -34
...
//.distance( 30)//function(d){console.log(d);return 1})//*eval(window.linkleneq)}) // 1-dval 500*0.4+(dv*dv*dv)/0.6
...
var link = svg.append("g")
  ...
  .enter().append("line")
    ...
    .style('stroke',(d)=> !d.new? color(1):'##F6089E');
...
var node = svg.append("g")
    ...
    .enter().append("circle")
      ...
      .attr("fill", (d)=> both.has(d.id)? 'url(#l'+d.id+')' : data.old.species.has(d.id)? 'blue':'orange' )// function(d) { return color(concs[d.id]); })
      .attr("id", (d)=> d.id)
      .attr('opacity', (d)=> !both.has(d.id)? 0.4 :1)
      .call(d3.drag()
