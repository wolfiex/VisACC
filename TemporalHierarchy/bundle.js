...
/***/ function(module, exports) {
	...
	function ColourScheme (colours, inverse = false, test=false){
	  var domain = [];
	  colours.forEach(function(d,i){if(inverse){i=colours.length-i};
	                                 domain.push(i/colours.length)}  )
	  var fn = d3.scaleLinear().domain(domain).range(colours).interpolate(d3.interpolateHcl);
	    if (test){
	...
	*/
	...
	svg.append("defs")
		...
		.attr("stop-color", function(d,i) { return cscheme( tempPoint[i] ); });
	...
	var legendsvg = svg.append("g")
		...
		.attr("transform", "translate(" + width/2 + "," + height*.97 + ")");
