...
function normalizeOptions(args) {
    ...
    function processTargets(platform, types) {
        ...
        function commonArch() {
            const result = Array();
            if (args.x64) {
                result.push(metadata_1.Arch.x64);
            }
            if (args.armv7l) {
                result.push(metadata_1.Arch.armv7l);
            }
            if (args.ia32) {
                result.push(metadata_1.Arch.ia32);
            }
            return result.length === 0 ? [metadata_1.archFromString(process.arch)] : result;
        }
...
function publishManager(packager, publishTasks, options, isPublishOptionGuessed) {
    ...
    packager.artifactCreated(event => {
        const publishers = platformPackager_1.getPublishConfigs(event.packager, event.packager.platformSpecificBuildOptions);
        // if explicitly set to null - do not publish
        if (publishers === null) {
            util_1.debug(`${ event.file } is not published: publish is set to null`);
            return;
        }
...
function createPublisher(packager, options, publishConfig) {
    let isPublishOptionGuessed = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;

    return __awaiter(this, void 0, void 0, function* () {
        const config = yield platformPackager_1.getResolvedPublishConfig(packager, publishConfig, isPublishOptionGuessed);
        if (config == null) {
            return null;
        }
        ...
        if (publishConfig.provider === "github") {
            ...
            log_1.log(`Creating Github Publisher — owner: ${ githubInfo.owner }, project: ${ githubInfo.repo }, version: ${ version }`);
            return new gitHubPublisher_1.GitHubPublisher(githubInfo.owner, githubInfo.repo, version, options, isPublishOptionGuessed, githubInfo);
        }
        if (publishConfig.provider === "bintray") {
            const bintrayInfo = config;
            log_1.log(`Creating Bintray Publisher — user: ${ bintrayInfo.owner }, package: ${ bintrayInfo.package }, repository: ${ bintrayInfo.repo }, version: ${ version }`);
