...
function run(t) {
  ...
  var nds = new Set(
    flatten(
      window.graph.links.map(d => {
        return [d.source, d.target];
      })
    )
  );
  window.graph.nodes = window.nodes.filter(d => {
...
window.graph.links = window.graph.links.sort(function(x, y){
  ...
  window.graph.links = window.graph.links.map((d, i) => {
    i < nm / 2
      ? d.v = Math.abs(i) * (2 / nm)
      : d.v = Math.abs(nm - i) * (2 / nm);
    console.log(d.v);
    return d;
  });
*/
...
function mcl() {
  ...
  fs.appendFile(file, "--->8------>8------>8------>8------>8---", function(
    err
  ) {
    if (err) throw err;
    console.log("Saved!");
...
function save() {
  ...
  fs.writeFile(
    "./locations.json",
    "data = " +
      JSON.stringify({ nodes: window.graph.nodes, links: window.graph.links }),
    function(err) {
      if (err) {
...
*/
    ...
    var f = "global.csv", fs = require("fs");
