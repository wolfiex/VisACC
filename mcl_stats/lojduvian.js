...
data.nodes.forEach(d => {
  ...
  document.getElementById("node" + d.id).style["fill-opacity"] = 1;
  document.getElementById("node" + d.id).style["stroke-opacity"] = 1;
  //document.getElementById("node" + d.id).r.baseVal.value = 3 *    parseFloat(result[d.id]);
});
...
data.links.forEach(d => {
  G.addEdge(d.source, d.target, { weight: 100 * d.v });
});
...
data.nodes.forEach(d => {
  ...
  document.getElementById("node" + d.id).r.baseVal.value = 10 *
    (ec.get(d.id) / max);
});

//// betweeness

var bn = jsnx.betweennessCentrality(G, optParameters = { weight: "weight" });
