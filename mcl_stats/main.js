...
app.on("ready", function() {
  //sync init con files
  var child = cp.exec("bash ./src/updateics.sh", function(
    error,
    stdout,
    stderr
  ) {
    console.log("updated ics");
    child.stdout.pipe(process.stdout);
  });
  cp.exec(
    "touch heatmap.html && touch locations.json && rm heatmap.html && rm locations.json"
  );
