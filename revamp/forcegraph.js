...
d3.json("./ics/Nhe_144.json", function(error, graph) { if (error) throw error; console.log(graph);window.graph=graph;run()});
...
//ipc.send('forwarder',['prefsWindow',window.linkleneq,dummy]);
  ...
  simulation.force("link")
        .links(graph.links)
        .distance(function(d){var dv= d.value;return 1*eval(window.linkleneq)}) // 1-dval 500*0.4+(dv*dv*dv)/0.6
...
graph_3d = JSON.parse(JSON.stringify(window.graph))

//  introduce a 3d force function, but constrain x and y, so remove x and y calculations?

//=> rewrite force function

window.simulation.vertx = d3.forceSimulation()
    //.force("charge", d3.forceManyBody().strength(-30))//- 18 -550 -32 -34
    .force("link", d3.forceLink().id(function(d) { return parseFloat(d.id); }))
    .force("center", d3.forceCenter(0,0))
    ...
    simulation.force("link")
          .links(graph_3d.links)
          .distance(function(d){var dv= d.value;return 10*eval(window.linkleneq)})
          .strength(function(d){var dv = d.value;return 0.9});


graph_3d.nodes.forEach(function(d){d.z=d.x});
    ...
    simulation.vertx
        ...
        function ticked() {
                ...
                graph_3d.nodes.forEach(function(d,i){d.y = JSON.parse(JSON.stringify(graph.nodes[i].y));graph.nodes[i].z = d.x; return d })
