...
function resize (){
  ...
  var xmultiplier = (0.9*window.innerWidth)/Math.abs(d3.min(x)-d3.max(x));
  var ymultiplier = (0.9*window.innerHeight)/Math.abs(d3.min(y)-d3.max(y));
  console.log('scaling y,x', ymultiplier,xmultiplier);
  var max = d3.min([xmultiplier,ymultiplier]);

  d3.transition().delay(1000)
  .duration(7000)
  .tween('strength', function() {
    var i = d3.interpolate(1.1,max);
    return function(t) {
    simulation.force("link").distance( function(d) {return 110*i(t) * d.v} ).iterations([9]).strength(1)};
  });


  simulation.force('collide', d3.forceCollide().radius(function(d){return  10}))
  //simulation.force("charge", d3.forceManyBody().strength(function(d){return (concs[d.id] > 0 )? charge*1.2   : 0  }).theta([40]));


  d3.transition().delay(9000)
  .duration(8000)
  .tween('strength', function() {
    var i = d3.interpolate(1,40);
    var j = d3.interpolate(1,1.2);
    return function(t) {
    simulation.force("charge", d3.forceManyBody().strength(function(d){return (concs[d.id] > 0 )? charge*j(t)  : 0  }).theta([i(t)]));
  }
  });
...
function setupvor(min=[0.1*width, .1*height],max=[width *.9, height *.9]){
  ...
  voronoi_path.append("circle").attr("clip-path", function(d, i) { return "url(#clip-" + i + ")"; });
...
function labels() {
  voronoi.polygons(nodes).map(function(d){  //return [d3.polygonCentroid(d),d.data.names]
...
function drawLink() {
  graphlinks.forEach(function(d){
    ...
    context.lineTo(d.target.x, d.target.y);
    context.strokeStyle =window.color(d.v).replace(')','').replace('rgb','rgba')+ ','+ 0.9 + ')';
    context.lineWidth= 0.1 + 1*(0.2+(1-d.v)/3);

    if (d.dir === 0){   context.setLineDash([5, 2]);  }
    context.filter=''
    context.stroke(); // draw stroke
    context.closePath();
