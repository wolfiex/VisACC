...
window.nodes = window.C.map( function(i){ return {"names":i, "id":window.ncdata.dict[i],"x":2*(0.5-Math.random()),"y":2*(0.5-Math.random()),"z":0.5-Math.random()}  } );
...
window.ncdata.combine.forEach(function(f){
  ...
  loss.forEach(function(i){var j = edge_len[i]; if (isFinite(j)){ flx -= j}});
